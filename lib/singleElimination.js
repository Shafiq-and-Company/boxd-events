...
export const handleSingleEliminationMatchCompletion = (bracketData, completedMatch, winnerId) => {
  ...
  console.log(`Match completion: Round ${completedMatch.round_number}, Winner: ${winnerId}, Loser: ${loserId}`);
  ...
  // Advance winner to next round (only if not finals)
  ...
  if (remainingPlayers.length === 1) {
    ...
    console.log(`Tournament complete! Winner: ${remainingPlayers[0].username || 'Unknown'}`);
  }
...
const advanceWinnerToNextRound = (bracketData, winnerId, completedMatch) => {
  ...
  // Check if this is the finals round - no advancement needed
  if (currentRound >= bracketData.totalRounds) {
    console.log('This is the finals round - no advancement needed');
    return; // Finals round - no next round to advance to
  }
  ...
  if (nextMatch) {
    ...
    console.log(`Available match:`, availableMatch);
    
    if (availableMatch) {
      if (!availableMatch.player1) {
        availableMatch.player1 = { id: winnerId };
        console.log(`Assigned winner ${winnerId} to player1 position`);
      } else if (!availableMatch.player2) {
        availableMatch.player2 = { id: winnerId };
        console.log(`Assigned winner ${winnerId} to player2 position`);
      }
    }
  } else {
    console.log('No next match found for advancement');
  }
};
 ...
 * @param {Object} supabaseClient - Supabase client instance
 */
export const updateDatabaseMatches = async (tournamentId, bracketData, supabaseClient = supabase) => {
  try {
    // Get all current matches for this tournament
    const { data: currentMatches, error: fetchError } = await supabaseClient
      .from('tournament_matches')
      .select('*')
      .eq('tournament_id', tournamentId);
    ...
    // Update matches with new player assignments from bracket data
    for (const round of bracketData.rounds) {
      if (round.matches) {
        for (const match of round.matches) {
          const dbMatch = currentMatches?.find(m => m.match_id === match.matchId);
          if (dbMatch) {
            const updates = {};
            if (match.player1?.id !== dbMatch.player1_id) {
              updates.player1_id = match.player1?.id || null;
            }
            if (match.player2?.id !== dbMatch.player2_id) {
              updates.player2_id = match.player2?.id || null;
            }
            if (match.winner !== dbMatch.winner_id) {
              updates.winner_id = match.winner || null;
            }
            if (match.status !== dbMatch.status) {
              updates.status = match.status || 'scheduled';
            }
            ...
            if (Object.keys(updates).length > 0) {
              ...
              if (error) {
                ...
                console.log(`Updated match ${match.matchId} with new assignments`);
              }
            }
          }
        }
    ...
    console.error('Error updating database matches:', error);
    throw error;
  }
};
