...
class TournamentManager {
  ...
  // Create tournament (single or double elimination)
  async createTournament(tournamentId, tournamentName) {
    ...
    // Get tournament format from database
    ...
    const format = tournamentData?.tournament_type || 'single_elimination';
    ...
    // Add double elimination specific settings
    if (format === 'double_elimination') {
      // Always use 'double' for grand finals reset (standard double elimination behavior)
      // If loser's bracket winner wins first grand final match, a reset match is played
  ...
  async updateMatch(tournamentId, matchId, opponent1Score, opponent2Score) {
    ...
    // Update the match
    // For double elimination with grandFinal: 'double', brackets-manager.js will automatically:
    // 1. Create the grand finals reset match if loser's bracket winner wins first grand final
    // 2. Advance winners properly through winner's and loser's brackets
    // 3. Handle all bracket progression logic
