...
class TournamentManager {
  ...
  async createTournament(tournamentId, tournamentName) {
    console.log(`Creating tournament ${tournamentId} with name: ${tournamentName}`);
    ...
    if (participants.length < 2) {
      ...
      console.log(`Tournament ${tournamentId} created with ${participants.length} participants. Waiting for more participants.`);
      
      // Update tournament status to indicate it's waiting for participants
      await supabase
        .from('tournaments')
        .update({ 
